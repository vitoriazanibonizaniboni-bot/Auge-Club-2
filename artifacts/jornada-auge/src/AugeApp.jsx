...
function MotivBanner({ ckOk, streakAtual, diasSemTreino, ir }) {
 ...
 flexShrink: 0,
        ...
        >
          <div
 style={{ fontFamily: FB, fontSize: 22, color: cor, lineHeight: 1 }}
          >
            {streakAtual}
          </div>
          <div
...
function RetroModal({ onFechar, regs, sem, registrarHabito, desregistrarHabito, habStats }) {
 ...
 return (
    <div onClick={onFechar} style={{ position: "absolute", inset: 0, zIndex: 400, background: "rgba(28,26,23,.87)", display: "flex", alignItems: "flex-end" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: C.creme, borderRadius: "20px 20px 0 0", padding: "22px 20px 34px", maxHeight: "80%", overflowY: "auto" }}>
        ...
        <div style={{ fontFamily: FB, fontWeight: 400, fontSize: 16, color: C.lt, lineHeight: 1.5, marginBottom: 14 }}>
 Esqueceu de marcar? Sem culpa — dá pra preencher até 7 dias pra trás. Tudo recalcula sozinho.
        ...
        {dias.map((ds) => (
          <div key={ds} style={{ background: C.linho, borderRadius: 10, padding: "11px 13px", marginBottom: 9 }}>
            <div style={{ fontFamily: FB, fontWeight: 400, fontSize: 16, color: C.terra, textTransform: "capitalize", marginBottom: 8 }}>{fmt(ds)}</div>
 ...
 style={{ flex: 1, background: feito ? `${C.ouro}30` : "transparent", border: `1px solid ${feito ? C.ouro : C.ouro + "40"}`, borderRadius: 8, padding: "8px 4px", fontFamily: FB, fontWeight: 400, fontSize: 13, color: feito ? C.ouroTxt : C.lt, cursor: "pointer" }}>
                    {feito ? "✓ " : ""}{h.nome}
                  </button>
                );
              })}
...
function Trajetoria({ regs, metas, kitUsos, sem, jornadaInicio, dataCadastro, historico = {}, ir, habsPessoais = [], anc, habProgresso, setHabProgresso }) {
 ...
 const [mx, my] = bez(tArc);
  ...
  // ── Lista de hábitos (seção 7) ────────────────────────────────────────────
  // Contagem do MÊS, de propósito: sequência de dias seguidos zera toda hora
  // numa meta de 3x por semana, e um contador sempre em zero desanima.
 ...
 bloqueado: sem < h.unlock,
 diasNoMes: diasDoMesFeitos(h.id),
    })),
    ...habsPessoais.map((hp) => ({
 ...
 return (
    <div style={{ animation: "fadeUp .35s ease" }}>
      <div style={{ background: C.creme, padding: "14px 18px 14px", position: "relative", borderBottom: `1px solid ${C.ouro}20` }}>
        <div style={{ fontFamily: FB, fontSize: 26, fontWeight: 600, color: C.obs, letterSpacing: "-0.01em" }}>
 Trajetória
        </div>
        <div style={{ fontFamily: FB, fontWeight: 400, fontSize: 16, color: C.lt, marginTop: 4 }}>
 ...
 S{sem}
        ...
        {/* ── Seus hábitos (seção 7) — contagem do mês, não sequência ── */}
        <div style={{ fontFamily: FB, fontWeight: 600, fontSize: 13, color: C.ouroTxt, letterSpacing: "0.18em", textTransform: "uppercase", margin: "6px 0 10px" }}>
 Seus hábitos
        </div>
        {linhas.map((h) => (
          <button
 key={h.id}
 onClick={() => !h.bloqueado && setHabSel(h)}
